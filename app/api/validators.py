...
from .exeptions import NotFound, Conflict
...
async def check_quantity_product(obj_create_order: OrderCreate, session: AsyncSession):
    ...
    if required_quantity > product.quantity:
        raise Conflict(detail=f'Заказ не может быть обработан: на складе недостаточно товара: {product.name}. Возможное количество для заказа: {product.quantity}.')
